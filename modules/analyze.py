...
def estimate_distance(detection, frame_height):
    ...
    if label.startswith('Driver Face'):
        return None
...
def analyze_hazards(detections, frame_shape, high_threshold=8.0, med_threshold=15.0):
    ...
    # Sort detections so closest hazards are processed first, ignoring those without distance
    detections.sort(key=lambda x: x['distance'] if x['distance'] is not None else float('inf'))
    return detections
